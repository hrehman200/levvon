...
<script type="text/javascript">
    ...
    function createChatBox(chatboxtitle,chatuserName, minimizeChatBox) {
        ...
        //call
        $.ajax({
            url: "<?=admin_url('auth/chat?action=chathistory')?>",
            cache: false,
    ...
    function chatHeartbeat(){
        ...
        $.ajax({
            url: "<?=admin_url('auth/chat?action=chatheartbeat')?>",
            cache: false,
            dataType: "json",
            success: function(data) {
    ...
    function closeChatBox(chatboxtitle) {
        ...
        $.post("<?=admin_url('auth/chat?action=closechat')?>", {
    ...
    function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle, boxUserName) {
        ...
        if(event.keyCode == 13 && event.shiftKey == 0)  {
            ...
            if (message != '') {
                $.post("<?=admin_url('auth/chat?action=sendchat')?>", {
                    to: chatboxtitle,
                    to_name: boxUserName,
                    message: message,
                    <?=$this->security->get_csrf_token_name()?>:"<?=$this->security->get_csrf_hash()?>"
    ...
    function startChatSession(){
        $.ajax({
            url: "<?=admin_url('auth/chat?action=startchatsession')?>",
            cache: false,
            dataType: "json",
            type:'GET',
            success: function(data) {
